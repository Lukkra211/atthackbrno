...
def outOfService(strip, delay=1):
    while True:
	colorWipe(strip, Color(155,155,0))
	time.sleep(delay)
	colorWipe(strip, Color(0,0,0))
	time.sleep(delay)

def yellow(strips, delay=3):
    print("*** Yellow! STOP! or Get ready! ***")
    for tl in strips:
    	colorWipe(tl, Color(255,200,0)) #Yellow
    time.sleep(delay)
